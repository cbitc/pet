...
> **先读什么**：完整文档地图见 [docs/README.md](docs/README.md)。速览——
> 领域说明书 [docs/domain.md](docs/domain.md)、技术架构 [docs/architecture.md](docs/architecture.md)、
> 项目规范 [docs/conventions.md](docs/conventions.md)、变更记录 [docs/issues/](docs/issues/README.md)。
...
docs/               文档：入口 README / 领域 / 架构 / 规范 / 协议 / issues 记录
```
...
- **透明合成修复（重要）**：本机环境需 `premultipliedAlpha:false`，否则 WebGL 内容出现后
  整窗变白（详见 [docs/issues/0001-white-screen.md](docs/issues/0001-white-screen.md)；
...
- **Live2D 纹理 GC 规避（重要）**：Pixi 8.15+ 默认约 60s 回收「未使用」纹理，而 Live2D
  ...
  `gcActive:false`，换形象时显式销毁纹理（详见 [docs/issues/0002-live2d-texture-gc.md](docs/issues/0002-live2d-texture-gc.md)）。
